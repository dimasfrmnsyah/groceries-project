...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    ...
    <div class="ms-auto">
        <a href="{{ route('accounting.supplier-debts.create') }}" class="btn btn-primary">Tambah Hutang</a>
    </div>
</div>

<h6 class="mb-0 text-uppercase">Hutang Supplier</h6>
<hr>
                ...
                <tbody>
                    @forelse($rows as $row)
                        <tr>
                            ...
                            <td class="text-end">{{ number_format($row->budget_amount, 0, ',', '.') }}</td>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    @if($row->status !== 'paid')
                                        <a href="{{ route('accounting.supplier-debts.payment', $row->id) }}" class="btn btn-sm btn-primary">Bayar</a>
                                    @endif
                                    <a href="{{ route('accounting.supplier-debts.edit', $row->id) }}" class="btn btn-sm btn-success"><i class="bx bx-pencil me-0"></i></a>
                                    <form method="POST" action="{{ route('accounting.supplier-debts.destroy', $row->id) }}" onsubmit="return confirm('Hapus hutang supplier ini?')">
                                        @csrf
                                        @method('DELETE')
